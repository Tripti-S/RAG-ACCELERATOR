...
# Adapted for week-2/scripts from week2_chunking/deduplication/01_deduplicate_documents.py
...
def write_selection_manifest(manifest_file: Path, all_files: List[Path], version_result: Dict, minhash_result: Dict):
    ...
    selected_files = []
    unique_files = minhash_result['unique_files']

    for f in unique_files:
        selected_files.append({
            "filepath": str(f),
            "selected": True,
            "content_type": "documentation",  # adjust if needed
            "source_dir": str(f.parent),
            "selection_reason": "minhash_unique"
        })
    ...
    manifest = {
        "manifest_metadata": {
            ...
            "reduction_percentage": round(
                (1 - len(selected_files)/len(all_files)) * 100, 2
            )
        },
