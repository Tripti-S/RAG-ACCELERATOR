...
Continuous Expectation and Variance, Quantiles,
...
Definition. 𝑋 continuous with range [𝑎, 𝑏] and pdf 𝑓(𝑥):
𝑏
𝐸[𝑋] = ∫ 𝑥𝑓(𝑥)𝑑𝑥.
𝑎
𝑋 discrete with values 𝑥 , …, 𝑥 and pmf 𝑝(𝑥 ):
1 𝑛 𝑖
𝑛
𝐸[𝑋] = ∑ 𝑥 𝑝(𝑥 ).
𝑖 𝑖
...
Definition. For any random variable 𝑋 with mean 𝜇,
Var(𝑋) = 𝐸[(𝑋 − 𝜇)2], 𝜎 = √Var(𝑋)
𝑋 continuous with range [𝑎, 𝑏] and pdf 𝑓(𝑥):
𝑏
Var(𝑋) = ∫ (𝑥−𝜇)2𝑓(𝑥)𝑑𝑥.
𝑎
𝑋 discrete with values 𝑥 , …, 𝑥 and pmf 𝑝(𝑥 ):
...
1. 𝐸[𝑋 + 𝑌 ] = 𝐸[𝑋] + 𝐸[𝑌 ].
2. 𝐸[𝑎𝑋 + 𝑏] = 𝑎𝐸[𝑋] + 𝑏.
3. If 𝑋 and 𝑌 are independent then
Var(𝑋 +𝑌) = Var(𝑋) + Var(𝑌 ).
4. Var(𝑎𝑋 + 𝑏) = 𝑎2Var(𝑋).
5. Var(𝑋) = 𝐸[𝑋2] − 𝐸[𝑋]2.
5/17
...
(d) Suppose 𝑋 , … 𝑋 are independent identically-distributed copies
1 16
of 𝑋. Let 𝑋 be their average. What is the standard deviation of 𝑋?
(e) Suppose 𝑌 = 𝑋4. Compute 𝐸[𝑌 ]
(f) Find the pdf of 𝑌 .
6/17
Quantiles: measure of location
Example. The 0.6 quantile 𝑞 is the 𝑥-value, with
0.6
𝑃 (𝑋 ≤ 𝑞 ) = 𝐹 (𝑞 ) = 0.6.
0.6 0.6
𝑓(𝑧)
lefttailarea=prob. =0.6
𝑥
𝑞 =0.253
0.6
𝐹(𝑥)
...
𝑞 =0.253
0.6
𝑞 : left tail area = 0.6 ⇔ 𝐹 (𝑞 ) = 0.6 ⇔ 𝑞 = 𝐹 −1(0.6)
0.6 0.6 0.6
...
The median of the black plot is at 𝑞. Which density has the greatest
median?
(A)
Curvescoincidetohere.
q
1. Black 2. Orange 3. Blue
...
• Formally: Let 𝑋 , 𝑋 , …be i.i.d. random variables all with mean
1 2
...
𝑛 𝑛 𝑛 𝑖
𝑖=1
Then for any (small number) 𝑎, we have
lim 𝑃(|𝑋 −𝜇| < 𝑎) = 1.
𝑛
𝑛→∞
• By choosing 𝑛 large enough we can, with probability close to 1,
make 𝑋 as close as we want to 𝜇.
𝑛
𝑋 is random, so there may be a small probability that it is far
𝑛
from 𝜇.
14/17
...
• If you bet $k, you either win $k with probability 𝑝 or lose $k with
probability 1 − 𝑝.
