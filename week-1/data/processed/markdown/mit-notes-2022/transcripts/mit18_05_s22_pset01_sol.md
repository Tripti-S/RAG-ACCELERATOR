...
13
1. Choose 2 ranks –one for each pair: ( ) ways.
2
4 4
2. Choose 2 cards from each chosen rank: 4 cards per suit: ( )( ) ways.
2 2
11
3. Choose 2 of the remaining ranks for the remaining 2 cards in the hand: ( ) ways.
2
4 4
4. Choose 1 card from each of these ranks: ( )( ) ways
1 1
...
13
1. Choose the rank of the triple: ( ) ways.
1
4
2. Choose 3 cards from the chosen rank: 4 cards per suit: ( ) ways.
3
1
18.05 Problem Set 1, Spring 2022 Solutions 2
12
3. Choose 3 of the remaining ranks for the remaining 3 cards in the hand: ( ) ways.
3
4 4 4
4. Choose 1 card from each of these ranks: ( )( )( ) ways
1 1 1
...
Probability 5/6 1/6 1/2 1/2 1/6 5/6
...
18.05 Problem Set 1, Spring 2022 Solutions 3
...
White Orange
2 5 1 4
Blue 3 5/12 5/12 5/36 25/36
6 1/12 1/12 1/36 5/36
Orange 1 1/12 1/12
4 5/12 5/12
...
2 Blue die 2 White die
Outcomes 6 9 12 4 7 10
Probability 25/36 10/36 1/36 1/4 1/2 1/4
2 White
4 7 10
6 25/144 50/144 25/144
2 Blue 9 10/144 20/144 10/144
12 1/144 2/144 1/144
Summing the different entries, We see, 𝑃 (white wins) = 85/144 (so 𝑃 (blue wins) = 59/144).
...
are equally likely to fall on any day of the year. Consider a group of 𝑛 people, of which you
are not a member. An element of the sample space 𝑆 will be a sequence of 𝑛 birthdays (one
for each person).
(a) Define the probability function 𝑃 for 𝑆. (This will depend on 𝑛.)
Solution: The sample space 𝑆 is the set of all sequences of 𝑛 birthdays. That is, all
sequences
𝜔 = (𝑏 , 𝑏 , 𝑏 , … , 𝑏 ),
1 2 3 𝑛
where each entry is a number between 1 and 365.
There are 365𝑛 sequences of 𝑛 birthdays. Since they are all equally likely, 𝑃 (𝜔) = 1 for
365𝑛
every sequence 𝜔.
18.05 Problem Set 1, Spring 2022 Solutions 4
...
Carefully describe the subset of 𝑆 that corresponds to each event.
Solution: Event 𝐴: Suppose my birthday is on day 𝑏. Then “an outcome 𝜔 is in 𝐴” is
equivalent to “𝑏 is in the sequence for 𝜔”, i.e. 𝑏 = 𝑏 for some index 𝑘 between 1 and 𝑛.
𝑘
More symbolically,
an outcome 𝜔 is in 𝐴 if and only if 𝑏 = 𝑏 for some index 𝑘 in 1, … , 𝑛.
𝑘
Event 𝐵: “An outcome 𝜔 is in 𝐵” is equivalent to “two of the entries in 𝜔 are the same”. That
is, an outcome 𝜔 is in 𝐵 if and only if 𝑏 = 𝑏 for two (different) indices 𝑗, 𝑘 in 1, … , 𝑛.
𝑗 𝑘
Event 𝐶: an outcome 𝜔 is in 𝐶 if and only if 𝑏 = 𝑏 = 𝑏 for three (distinct) indices
𝑗 𝑘 𝑙
𝑗, 𝑘, 𝑙 in 1, … , 𝑛.
(c) Find an exact formula for 𝑃 (𝐴). What is the smallest 𝑛 such that 𝑃 (𝐴) > 0.5?
Solution: It’s easier to calculate 𝑃 (𝐴𝑐). Since there are 364 birthdays that are not mine,
there are 364𝑛 outcomes in 𝐴𝑐.
364𝑛
𝑃(𝐴) = 1−𝑃(𝐴𝑐) = 1− .
365𝑛
We can find the size of the group needed for 𝑃 (𝐴) > 0.5 by trial and error, plugging in
different values of 𝑛. Or we can set 𝑃 (𝐴) = 0.5 and solve for 𝑛.
364𝑛 364 𝑛 364
1 − = 0.5 ⇒ ( ) = 0.5 ⇒ 𝑛 ⋅ ln ( ) = ln(0.5) ⇒ 𝑛 ≈ 252.65
365𝑛 365 365
So there needs to be at least 253 people for it to be more likely than not that one of them
shares your birthday.
(d) Justify why 𝑛 in part (c) is greater than 365 without doing any computation. (We are
2
...
(e) Use R simulation to estimate the smallest 𝑛 for which 𝑃 (𝐵) > 0.9. For these simulations,
let the number of trials be 10000. (You can reuse your code from Studio 1.)
For this value of 𝑛, repeat the simulation a few times to verify that it always gives similar
results.
Using 10000 trials you saw very little variation in the estimate of 𝑃 (𝐵). Try this again
using 30 trials and verify that the estimated probabilities are much more variable. On your
pset, give the results 7 runs of 30 trials using the value of 𝑛 you just found.
Solution: Here’s the R code I ran to estimate 𝑃 (𝐵).
...
18.05 Problem Set 1, Spring 2022 Solutions 5
...
𝑃 (𝐵).
npeople 𝑃 (𝐵) (multiple values means multiple runs of the code)
20 0.4123
30 0.7007
40 0.8913, 0.8867, 0.8926, 0.8865
41 0.9005, 0.9003, 0.9037, 0.9006, 0.903
...
(f) Find an exact formula for 𝑃 (𝐵).
Solution: It’s easier to calculate 𝑃 (𝐵𝑐), the probability that all 𝑛 birthdays are distinct.
To choose 𝑛 different birthdays: there are 365 choices for the first birthday, 364 for the
second birthday, etc. So
365 ⋅364⋯(365−𝑛+1) 365!
𝑃(𝐵) = 1−𝑃(𝐵𝑐) = 1− = 1− .
365𝑛 (365 − 𝑛)! ⋅ 365𝑛
(g) Use R simulation to estimate the smallest 𝑛 for which 𝑃(𝐶) > 0.5. Again use 10000
trials. You will find that two adjacent values of 𝑛 are equally plausible based on simulations.
You may pick either one for your answer.
Note that it is much harder to find an exact formula for 𝑃 (𝐶), so simulation is especially
handy.
Solution: To estimate 𝑃 (𝐶) we used the same code as in part (e), except we set size_match
= 3. Here is the table of (estimated) probabilities we found.
18.05 Problem Set 1, Spring 2022 Solutions 6
npeople 𝑃 (𝐶) (multiple values means multiple runs of the code)
20 0.0121
30 0.0293
50 0.1274
80 0.4241
...
85 0.4832
86 0.4841
87 0.502, 0.4989, 0.4909, 0.5035
88 0.5081, 0.5115, 0.5149, 0.5071
The estimates for 87 are sometimes above and sometimes below 0.5. So the answer to the
question is either 87 or 88. It seems certain that with 88 people 𝑃(𝐶) > 0.5.
MIT OpenCourseWare
https://ocw.mit.edu
