...
Problem 1. A full house in poker is a hand where three cards share one rank and two
...
Problem 8. Let 𝐴 and 𝐵 be two events. Suppose the probability that neither 𝐴 or 𝐵
...
Problem 12. More cards! Suppose you want to divide a 52 card deck into four hands
with 13 cards each. What is the probability that each hand has a king?
Problem 13. Suppose you are taking a multiple-choice test with 𝑐 choices for each
question. In answering a question on this test, the probability that you know the answer is
𝑝. If you don’t know the answer, you choose one at random. What is the probability that
you knew the answer to a question, given that you answered it correctly?
Problem 14. Corrupted by their power, the judges running the popular game show
...
Problem 20. Some games, like tennis or ping pong, reach a state called deuce. This
means that the score is tied and a player wins the game when they get two points ahead
of the other player. Suppose the probability that you win a point is 𝑝 and this is true
...
Problem 24. You roll a twenty-sided die. Determine whether the following pairs of
...
Problem 28. The random variable 𝑋 takes values -1, 0, 1 with probabilities 1/8, 2/8,
5/8 respectively.
(a) Compute 𝐸[𝑋].
...
(a) Let 𝑋 be the amount of money bet on the last game (the one I win). 𝑋 takes values
1, 2, 4, 8, …. Determine the probability mass function for 𝑋. That is, find 𝑝(2𝑘), where 𝑘
is in {0, 1, 2, …}.
(b) Compute 𝐸[𝑋].
...
Problem 34. Suppose that 𝑋 ∼ Bin(𝑛, 0.5). Find the probability mass function of
𝑌 = 2𝑋.
...
Suppose 𝑋 and 𝑌 are independent and 𝑋 ∼ Bernoulli(1/2) and 𝑌 ∼ Bernoulli(1/3). De-
termine the pmf of 𝑋 +𝑌
...
Problem 47. Normal Distribution: Throughout these problems, let 𝜙 and Φ be the
pdf and cdf, respectively, of the standard normal distribution Suppose 𝑍 is a standard
normal random variable and let 𝑋 = 3𝑍 +1.
...
(c) Find 𝑃(−1 ≤ 𝑋 ≤ 1)
(d) Recall that the probability that 𝑍 is within one standard deviation of its mean is
approximately 68%. What is the probability that 𝑋 is within one standard deviation of its
mean?
...
Let 𝑋, 𝑌 be independent random variables where 𝑋 ∼ 𝑁(2,5) and 𝑌 ∼ 𝑁(5,9) (we use
the notation 𝑁(𝜇, 𝜎2)). Let 𝑊 = 3𝑋−2𝑌 +1.
...
the total number of heads on the last two tosses, and 𝐹 the number of heads on the first
two tosses.
...
Consider 𝑋 , 𝑋 , …all independent and with distribution N(0, 1). Let 𝑋 be the average
1 2 𝑚
of 𝑋 , …𝑋 .
1 𝑛
