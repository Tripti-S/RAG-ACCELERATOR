...
Solution: 3. likelihood
Both the prior and posterior are probability distributions on the possible values of the
unknown parameter 𝜃, i.e. a distribution on hypothetical values. The frequentist does not
consider them valid.
The likelihood 𝜙(𝑥|𝜃) is perfectly acceptable to the frequentist. It represents the probability
...
𝜎 / 𝑛 0 0
0
(a) Yes. The median only depends on the data 𝑥 , … , 𝑥 .
1 𝑛
(b) No. This interval depends only on the distribution parameters 𝜇 and 𝜎. It does not
consider the data at all.
(c) No. this depends on the values of the unknown parameters 𝜇 and 𝜎.
(d) Yes. 𝑥̄ depends only on the data, so the set of values within 1 of 𝑥̄ can all be found by
working with the data.
1
18.05 class 17 problems, Spring 2022 2
(e) Yes. This is computed from the data and other known values.
(f) Yes. This is computed from the data and other known values.
Concept question 2. Picture the significance. The null and alternate pdfs are shown
...
2 3 4
Solution: 7. 𝑅 + 𝑅 . This is the area under the pdf for 𝐻 above the rejection region.
2 3 0
Board questions
...
0
(a) The rejection region is |𝑧| > 1.96, i.e. 1.96 or more standard deviations from the mean.
𝑥−5 1.25
(b) Standardizing 𝑧 = = = 1.
5/4 1.25
...
0 0 0
(e) The 𝑧-value not being in the rejection region tells us exactly the same thing as the
𝑝-value being greater than the significance, i.e. don’t reject the null hypothesis 𝐻 .
0
...
18.05 class 17 problems, Spring 2022 4
k 0 1 2 3 4 5 6 7 8
...
Solution: (a) Since 0.6 > 0.5 we use a right-sided rejection region.
Under 𝐻 the probability of heads is 0.5. Using the table we find a one sided rejection
0
region {7, 8}. That is we will reject 𝐻 in favor of 𝐻 only if we get 7 or 8 heads in 8 tosses.
0 𝐴
Since the value of our data 𝑥 = 6 is not in our rejection region we do not reject 𝐻 .
0
(b) Since 0.6 > 0.5 we use a left-sided rejection region.
Now under 𝐻 the probability of heads is 0.6. Using the table we find a one sided rejection
0
region {0, 1, 2}. That is we will reject 𝐻 in favor of 𝐻 only if we get 0, 1 or 2 heads in 8
0 𝐴
tosses.
Since the value of our data 𝑥 = 6 is not in our rejection region we do not reject 𝐻 .
0
(c) The fact that we don’t reject 𝐶 in favor of 𝐶 or 𝐶 in favor of 𝐶 reflects the asymmetry
1 2 2 1
in NHST. The null hypothesis is the cautious choice. That is, we only reject 𝐻 if the data
0
is extremely unlikely when we assume 𝐻 . This is not the case for either 𝐶 or 𝐶 .
0 1 2
MIT OpenCourseWare
https://ocw.mit.edu
