# 63857C0C2D83D09F219A6F745Beaa557 Yvhu34Mexzk

---

MITOCW | MITRES6_012S18_S11-01_300k
Simulation is an important tool in the analysis of probabilistic phenomena.
For example, suppose that X, Y, and Z are independent random variables, and you're interested in the statistical
properties of this random variable.
Perhaps you can find the distribution of this random variable by solving a derived distribution problem, but
sometimes this is impossible.
And in such cases, what you do is, you generate random samples of these random variables drawn according to
their distributions, and then evaluate the function g on that random sample.
And this gives you one sample value of this function.
And you can repeat that several times to obtain some kind of histogram and from that, get some understanding
about the statistical properties of this function.
...
And random number generators, typically what they do is generate values that are drawn from a uniform
distribution.
...
What we want to do is to create some kind of box or function that takes this uniform random variable and
generates g of U.
...
If my uniform random variable falls in this range, which happens with probability 2/6, I'm going to report this value
for my discrete random variable.
Then I take an interval of length 3/6, which takes me to 5/6.
And if my uniform random variable falls in this range, then I'm going to report that value for my discrete random
variable.
...
So this is how we can generate random samples of a discrete distribution, starting from a uniform random
variable.
...
Now, the important thing is that because of our assumption that f is continuous and strictly monotonic, this inverse
...
If I take some number c and then take the corresponding number up here, which is going to be F_X of c, then we
have the following property.
My random variable X is going to be less than or equal to c if and only if my random variable X falls into this
interval.
...
So we have verified that with this way of constructing samples of X based on samples of U, the random variable
that we get has the desired CDF.
Let's look at an example now.
Suppose that we want to generate samples of a random variable, which is an exponential random variable, with
parameter 1.
...
If a u corresponds to 1 minus e to the minus x-- so we started with some x here and we find the corresponding u--
this is the formula that takes us from x's to u's.
...
And so we have found a way of simulating exponential random variables, starting with a random number
